...
# 配置文件：保留现场打印机设置；传入 ServerUrl 时仅写入或更新 [server] url
...
"@ | Set-Content $configPath -Encoding UTF8
    Write-Host "已写入默认配置 $configPath（服务器: $ServerUrl）"
} elseif ($ServerUrl) {
    $config = Get-Content $configPath -Raw -Encoding UTF8
    if ($config -match '(?m)^url\s*=') {
        $config = $config -replace '(?m)^url\s*=.*$', "url = `"$ServerUrl`""
    } elseif ($config -match '(?m)^\[server\]\s*$') {
        $config = $config -replace '(?m)^\[server\]\s*$', "[server]`r`nurl = `"$ServerUrl`""
    } else {
        $config = "[server]`r`nurl = `"$ServerUrl`"`r`n`r`n$config"
    }
    Set-Content $configPath -Value $config -Encoding UTF8 -NoNewline
    Write-Host "已更新服务器地址: $ServerUrl"
} else {
